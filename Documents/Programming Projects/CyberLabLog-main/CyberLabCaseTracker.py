...
class CaseLogApp:
    ...
    def create_about_widgets(self):
        ...
        about_text = (
            ...
            "What's New in v2.1.4 (Aug 28, 2025):\n"
            "- Date Range Reports: Added calendar pickers with quick actions (Last 30 Days, Current Month).\n"
            "- Export Flow: 'Generate' now lets you export directly as PDF or Text.\n"
            "- Settings: Report header editor has Save/Cancel and persists values.\n"
            "- Images: Logo and custom map marker persist in app_data; previews always show; logo on New Entry.\n"
            "- Database: Enabled WAL and busy timeouts; added update retry to avoid 'database is locked'.\n"
            "- Activity: Recent activities fallback uses case created_at when activity table is empty.\n"
            "- Stability: Defined geocache counters at startup to avoid NameError.\n\n"
            ...
            f"Version: {APP_VERSION} (Aug 28, 2025)\n"
    ...
    def _parse_version_tuple(self, s: str) -> tuple:
        """Extract a simple numeric version tuple from a tag like 'v2.1.4-20250828'."""
        try:
    ...
    def _is_remote_newer(self, remote_tag: str) -> bool:
        remote = self._parse_version_tuple(remote_tag)
        local = self._parse_version_tuple(APP_VERSION)
        return remote > local

    def _show_update_popup(self, title: str, body: str, url: str | None = None):
        """Show a scrollable popup with release notes and optional link."""
        win = tk.Toplevel(self.root)
        win.title(title)
        ...
        txt.insert('1.0', body or "No release notes provided.")
        ...
        if url:
            def open_url():
                try:
                    ...
                    pass
            ttk.Button(btns, text="Open Release Page", command=open_url).pack(side='left')
        ttk.Button(btns, text="Close", command=win.destroy).pack(side='right')

    def check_for_updates(self, silent: bool = True):
        """Check GitHub for a newer release and show changes if available.
        - When silent=True, only notifies on available update; otherwise shows up-to-date/info messages.
        """
        owner_repo = "RF-YVY/CyberLabLog"
        latest = None
        error = None
        try:
            # Prefer releases (with notes), fallback to tags
            resp = requests.get(f"https://api.github.com/repos/{owner_repo}/releases", timeout=6)
            if resp.status_code == 200:
                releases = resp.json()
                if releases:
                    # Assume first is latest by publish order
                    latest = releases[0]
                else:
                    # Fallback to tags
                    tags_resp = requests.get(f"https://api.github.com/repos/{owner_repo}/tags", timeout=6)
                    if tags_resp.status_code == 200:
                        tags = tags_resp.json()
                        if tags:
                            latest = {"tag_name": tags[0].get("name"), "html_url": f"https://github.com/{owner_repo}/releases"}
            else:
        ...
        if not latest:
            if not silent:
                Messagebox.show_warning("Update Check", f"Could not check for updates. {error or ''}".strip())
            return

        tag = latest.get("tag_name") or ""
        body = latest.get("body") or ""
        html_url = latest.get("html_url") or latest.get("url") or f"https://github.com/{owner_repo}/releases"

        if tag and self._is_remote_newer(tag):
            title = f"Update Available: {tag} (current {APP_VERSION})"
            notes = body.strip() or "A new version is available. Visit the release page to download."
            self._show_update_popup(title, notes, html_url)
        else:
            if not silent:
                Messagebox.show_info("Update Check", f"You're up to date. Current version: {APP_VERSION}.")
