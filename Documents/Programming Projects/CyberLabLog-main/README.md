...
- **Reports:** Export PDF/XLSX reports (full, summary, custom, or selected rows). Persistent report header info (agency, division, name, date) included in all reports; success dialogs show the full "Saved to:" path.
...
- **Reliability:** Automatic SQLite database backups on close with timestamped copies and 56-day retention.
- **Updates:** Notify-only update check via the About tab (also runs silently on startup).
- **About Tab:** Comprehensive app info, version display, update check button, and clickable developer GitHub link.
...
- Automatic backups are written to `app_data/backups` on close; older backups are pruned after ~56 days.
...
```
pip install ttkbootstrap tkintermapview matplotlib openpyxl reportlab geopy pandas requests
```
...
- The app creates a timestamped SQLite backup automatically when you close the application. Backups are stored under `app_data/backups` and older backups are automatically pruned (default retention ~56 days).

## Update Checks

- The About tab includes a "Check for Updates" button. On startup, the app also performs a silent notify-only check against GitHub Releases; if a newer version is found, you'll be prompted to visit the releases page.

## Building, Packaging, and Code Signing

- See `PACKAGING.md` for instructions to build with PyInstaller, optional code signing on Windows, and details about the update check mechanism.
